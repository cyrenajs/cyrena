...
import { castArray, compact, omit, mapValues, zip, merge, uniqueId, get, set, without, clonePath } from './fp.js';
import { isComponentNode, isElement, isStream, isInlineComponent } from './dynamictypes.js';
...
const makeTraverseAction = config => (acc, __val, path) => {
    ...
    const regularCmp = _isRegularCmp && resolveShorthandOutput(val.type) ||
        _isInlineCmp && resolveShorthandOutput(val);
    const cmp = _isCmp && handleAutoScope(regularCmp, val.props, config.vdomProp);
...
const makePowercycle = config => function powercycle(vdom, eventSinks, sources) {
    ...
    const signalStreams = placeholders.map(node => node.isCmp
    ...
    const vdom$ = config.combineFn(signalStreams)
        ...
        zip(signalValues, placeholders)
            ...
            if (isElement(val) && !val.key) {
                ...
                // the cmp id
                _val = { ..._val, key: get([...info.path, 'key'])(root) ||
    ...
    const allEventSinks = [placeholders]
        .map(xs => xs.filter(rec => rec.isCmp))
        .map(xs => xs.map(rec => rec.sinks))
        .map(xs => [eventSinks || {}, ...xs])
        .map(xs => xs.reduce((acc, next) => merge(acc, omit([config.vdomProp])(next), (addition, src) => compact([...castArray(addition), src])), {}))
        .map(mapValues(sinks => config.mergeFn(sinks)))[0];
    return {
...
export default Object.assign(Cmp => sources => powercycle(pragma(Cmp), null, sources), { pragma, Fragment, createElement: pragma });
//# sourceMappingURL=powercycle.js.map