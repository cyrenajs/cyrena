...
const EVENT_PROPS = ('Copy|Cut|Paste|CompositionEnd|CompositionStart|CompositionUpdate|KeyDown|' +
    ...
    'AnimationStart|AnimationEnd|AnimationIteration|TransitionEnd|Toggle').split('|').map(ev => 'on' + ev);
...
function resolveDomEventProps(vdom) {
    ...
    wrapVdom(vdom, getInlineCmp, Object.keys(eventProps), 
...
function resolveComponentEventProps(vdom, powercycle) {
    const eventProps = pickBy((...[, prop]) => /^on(?:$|-|[A-Z])/.test(prop))(vdom.props);
    const getTriplets = cfg => Object.keys(cfg).reduce((...[, channel]) => Object.keys(cfg[channel]).reduce((cum, event) => [...cum, [channel, event, cfg[channel][event]]], []), []);
    // channel, filter key, translation (payload-to-action fn)
    const triplets = Object.keys(eventProps).reduce((cum, next) => cum.concat(getTriplets(next === 'on'
        ? vdom.props['on'] :
        /^on-/.test(next)
    ...
    if (triplets.length > 0) {
        ...
        vdom.type = sources => {
            ...
            sinks.state = CONFIG.mergeFn(compact(triplets.map(([channel, event, payloadToAction]) => sinks[channel] && sinks[channel]
...
//# sourceMappingURL=resolveEventProps.js.map