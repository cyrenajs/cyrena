...
export const $and = (...conditions) => {
    ...
    return $map(state => combined(state).reduce(andReducer, true));
};
export const $combine = (...mappers) => {
    ...
    throw new Error('Powercycle/$combine: mixing state mappers and streams is not yet supported.');
};
export const $or = (...conditions) => {
    return $not($and(...conditions.map($not)));
};
export const $eq = (...args) => {
    return $map(values => values[0] === values[1], $combine(args[0], args.length > 1 ? args[1] : $));
};
