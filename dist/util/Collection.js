...
export const collection = (stream, options) => {
    const itemCmp = typeof options === 'function'
        ? options
        : options.itemCmp;
    const getKey = typeof options.itemKey === 'function'
        ? options.itemKey :
...
export const COLLECTION_DELETE = () => undefined;
export const CollectionItem = sources => wrapInComponent(sources.props.children)(sources);
const identityLens = {
    get: state => state,
    set: (...[, childState]) => childState
};
export function Collection(sources) {
    ...
    const listCmpWithSpecialItemScope = [collectionCmp]
        ...
        return isolate(list, {
            state: {
                ...
                set: (state, childState) => {
                    let outerStateChanged = null;
                    let collectionChanged = null;
                    const collection = forLens.get(state);
                    const setterForItemWrap = (state, childState) => childState.map(record => {
                    ...
                    const result = forLens.set(state, setterForItemWrap(state, childState));
        ...
        : list)[0];
    return pragma(listCmpWithSpecialItemScope, null, ...castArray(sources.props.children));
}
