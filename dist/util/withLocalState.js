...
export function _withLocalState(cmp) {
    ...
    const wrapper = function WithLocalState(sources) {
        ...
        const state$ = xs
            .combine(sources[stateChannel].stream.startWith(undefined), sources[localChannel].stream.startWith(undefined))
            .map(([g, l]) => merger.merge(g, l))
            .remember();
        const sinks = cmp({
            ...omit([localChannel])(sources),
            [stateChannel]: new StateSource(state$.drop(2), 'withLocalState')
        });
...
//# sourceMappingURL=withLocalState.js.map