...
export const mergeWith = src => obj => {
    return mergeDeep(obj, src, (...[, newVal]) => {
...
export function request(url$, sources) {
    ...
    const content$ = response$
        ...
        .remember();
    const isLoading$ = xs.merge(request$.mapTo(true), response$.mapTo(false)).startWith(false);
...
export const $get = (key, src) => $map(streamVal => key ? _get(key.split('.'))(streamVal) : streamVal, src);
