...
function resolveScopeProp(vdom, powercycle) {
    ...
    wrapVdom(vdom, (type, props, children) => isolate(resolveShorthandOutput(powercycle)(() => pragma(type, props, ...castArray(children))), resolvePathScope(vdom.props.scope)), ['scope'], {});
    return true;
}
function resolveIfProp(vdom, powercycle) {
    ...
    wrapVdom(vdom, (type, props, children) => sources => {
        return getConditionalCmp(resolveStateMapper(cond, sources), cond => sources => powercycle(cond
            ? pragma(type, props, ...castArray(children))
            : pragma(Fragment), null, sources))(sources);
    }, ['if'], {});
    return true;
}
...
//# sourceMappingURL=resolveScopeOrIfProp.js.map